-- ========================================
-- WORDHELPER V4 - ULTRA SAFE MODE
-- Script akan tetap berjalan apapun yang terjadi
-- ========================================
...
local function safeCall(func, errorMsg)
    ...
    if not success then
        warn("[WordHelper Error] " .. (errorMsg or "Unknown error") .. ": " .. tostring(err))
    end
    return success
end
...
-- Safe request function with multiple fallbacks
local request = (syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request
if not request then
    -- Create a dummy request function if none available
    request = function(options)
        warn("[WordHelper] HTTP request not available in this executor")
        return nil
    end
end

-- Safe file system functions
local safeWriteFile = writefile or function() warn("[WordHelper] writefile not available") end
local safeReadFile = readfile or function() warn("[WordHelper] readfile not available") return "" end
local safeIsFile = isfile or function() return false end
...
local function LoadConfig()
    if isfile and readfile and HttpService and isfile(ConfigFile) then
...
local function FetchWords()
    ...
    if success and res and res.Body and writefile then
        pcall(function()
...
local function LoadList(fname)
    ...
    if isfile and readfile and isfile(fname) then
        ...
        if success and content then
            ...
            UpdateStatus("Failed to read file!", Color3.fromRGB(255, 80, 80))
        end
    else
...
-- If no words loaded, create a minimal fallback list
if #Words == 0 then
    warn("[WordHelper] No words loaded! Creating minimal fallback list...")
    local fallbackWords = {"apple", "banana", "cat", "dog", "elephant", "fish", "grape", "house", "ice", "juice", "kite", "lemon", "mango", "nest", "orange", "pear", "queen", "rose", "star", "tree", "umbrella", "vase", "water", "xray", "yellow", "zebra"}
...
runConn = RunService.RenderStepped:Connect(function()
    -- Wrap entire loop in pcall to prevent crashes
    local success, err = pcall(function()
        local now = tick()
        local player = Players.LocalPlayer
        local gui = player and player:FindFirstChild("PlayerGui")
    ...
    if not success then
        warn("[WordHelper] Main loop error: " .. tostring(err))
    end
end)
...
print("[WordHelper V4] Script loaded successfully!")
print("[WordHelper V4] Press Right Control to toggle GUI")
print("[WordHelper V4] Total words loaded: " .. #Words)
